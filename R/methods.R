...
#' @rdname sctr
setMethod("sctr",signature(obj="binClst"),function(obj,ref=NULL,showVars=NULL,showClst=NULL,...){
	if (!is.null(ref)){			
		...
		if (obj@m==2){
			...
			parDef <- parSet(mtx=matrix(c(1,2,3),1,3,byrow=TRUE),widths=c(1,1,0.5),bg=brewer.pal(8,'Greys')[4],oma=c(4,1,4,1))
...
#' @rdname view
setMethod("view",signature(obj="binClstPath"),function(obj,lbl=NULL,lims=NULL,...){
	parDef <- parSet(mtx=matrix(c(1,2),2,1),heights=c(0.2,1),bg=brewer.pal(8,'Greys')[4])
	...
	par(mar=c(4,4,1,1))
	plot(1,1,xlab='lon',ylab='lat',xlim=c(min(obj@pth$lon[a:b]),max(obj@pth$lon[a:b])),ylim=c(min(obj@pth$lat[a:b]),max(obj@pth$lat[a:b])),type="n")
	points(obj@pth$lon[a:b],obj@pth$lat[a:b],col=pal[lbl[a:b]],pch=20)
	lines(obj@pth$lon[a:b],obj@pth$lat[a:b],col=pal[length(pal)])
#  	world(xlim=c(min(obj@pth$lon),max(obj@pth$lon)),ylim=c(min(obj@pth$lat),max(obj@pth$lat)))
 	par(parDef)
	})

#' @rdname view
setMethod("view",signature(obj="data.frame"),function(obj,lbl=NULL,lims=NULL,...){
	parDef <- parSet(mtx=matrix(c(1,2),2,1),heights=c(0.2,1),bg=brewer.pal(8,'Greys')[4])
	...
	rect(seq(a,b-1),0,seq(a+1,b),1,col=pal[lbl[a:b]],border=NA)
	par(mar=c(4,4,1,1))
	plot(1,1,xlab='lon',ylab='lat',xlim=c(min(obj$lon[a:b]),max(obj$lon[a:b])),ylim=c(min(obj$lat[a:b]),max(obj$lat[a:b])),type="n")
	points(obj$lon[a:b],obj$lat[a:b],col=pal[lbl[a:b]],pch=20)
	lines(obj$lon[a:b],obj$lat[a:b],col=pal[length(pal)])
#  	world(xlim=c(min(obj$lon),max(obj$lon)),ylim=c(min(obj$lat),max(obj$lat)))
 	par(parDef)
	})
