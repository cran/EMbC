...
date: "Version 1.9.3, `r format(Sys.Date(), '%b %Y')`"
output:
  pdf_document:
    number_sections: yes
    toc: no
...
\bigskip
\begin{abstract}
In this document we give a brief overview on the use of the EMbC R-package with special emphasis on its use for behavioural annotation of animal's movement trajectories. For details about the EMbC algorithm please refer to (Garriga et. al 2015) and for further details about the package please refer to the package reference manual.
\end{abstract}
\bigskip
...
Also, for researchers who are familiar with the MoveBank framework, we include a special dependence with the package **move**, *Visualizing and analizing animal track data* [@move], to allow users to make use of *Move* objects with its full functionality.
...
Basically, the package consists of a hierarchy of classes:
  ...
  - *binClstMove*, a class that simultaneously inherits all functionality of a *binClstPath* and a *Move* object from the **move** R-package;
  - *binClstStck*, basically a list of *binClstPath* or *binClstMove* objects resulting from the global clustering of a stack of trajectories.
...
This is the core class implementing the multivariate binary clustering algorithm. The input data-set is given as a matrix with data points given as rows and input features as columns. No more than 5 (6 at most) variables should be used in order to get a human interpretable set of clusters.

Let's use the object *x2d* included in the package. This object contains a set of bivariate data points generated from a GMM with four components (slot x2d@D), and a labelling indicating which component generated each data point (slot x2d@L);
...
To perform the clustering of this data-set we simply call the *embc()* constructor passing in a matrix with the input data and naming the output *binClst* object (e.g. mybc);

```{r, }
mybc <- embc(x2d@D)
```

At each step, the algorithm shows the current iteration number, the likelihood value, the number of effective clusters and the number of data points that have changed their labels with respect to the previous iteration.
...
The most basic slots of a *binClst* object are:
  ...
  - mybc@P, a list with an element for each cluster, where each element is a named list with the Gaussian parameters of the corresponding Gaussian component;
...
We can check the convergence of the algorithm by plotting the likelihood values at each step;

```{r, fig.width=3.8, fig.height=3, fig.show='hold'}
...
The function *stts()* shows the mean $X_{i}.mn$ and standard deviation $X_{i}.sd$ of the input features in each cluster. The last two columns show the marginal distribution of the cluster in absolute and percentage values;
...
The *delimiters* are also accessible through slot mybc@R where we have the *min()* and *max()* values that delimit each binary region;
...
We can also perform a numeric assessment of the clustering in terms of a confusion matrix by means of the function *cnfm()*;

```{r, }
cnfm(mybc,x2d@L)
```

The confusion matrix shows values of row *precision* and row *F-measure*, and values of column *recall* and column *F-measure*. The (3,2) subset of cells at the bottom-right show respectively: the *overall accuracy*, the *average recall*, the *average precision*, NaN, NaN and the *Macro-F-measure*.
...
To perform the bivariate velocity/turn clustering of this trajectory we simply call the *stbc()* constructor passing in the data.frame with the time/space coordinates of the trajectory and naming the output *binClstPath* object (*e.g.* *mybcp*);
...
Note that *expth* is a synthetically generated trajectory with expert labelling (note the column expth\$lbl). Further columns of data can be included in the input data.frame as long as the first three columns respect the required format. Particularly, if an expert labelling is included in a column labelled *$lbl*, all validation functions will make use of it by default.
...
As a child class, a *binClstPath* object inherits and extends the set of slots of the *binClst* class. The basic differences with respect to the *binClst* class are:
...
Nonetheless, the *binClstPath* class has some particular functionalities of special interest for the case of behavioural annotation of movement trajectories.

### Labeling profile

The function *lblp()* depicts the temporal series of data along with the temporal profile of the behavioural labelling;
...
The function *view()* shows the annotated trajectory with a top panel depicting the temporal sequence of behaviours;
...
We can generate *kml* or *html* documents for a detailed inspection of the output by means of *google-earth* or the user's system browser. The package allows two types of visualization of the annotated trajectory: a point-wise visualization (functions *pkml()* or *pmap()*) or a burst-wise visualization [@embc2015] (functions *bkml()* or *bmap()*);
...
```{r, fig.width=5, fig.height=4.2, fig.align='center'}
...
Indeed, the function *varp()* is a wrapper for the R *plot()* function and can be used to plot any numeric vector. The only advantage is that when the vector is recognized as self data axes are automatically formatted and labelled and thus it provides a fast means for the inspection of intermediate data.

# Class: binClstMove

The *binClstMove* is a class that inherits all functionality of a *binClstPath* class and of a *Move* object from the R-package *move*. This is intended for users having trajectories in *Movebank* (<https://www.movebank.org/>) and familiarized with the *move* R-package. *Move* objects can be passed directly to the *stbc()* constructor.

Let's use the example given in the *Move R-package* to spell out further functionality of the package associated to both *binClstPath* and *binClstMove* objects;
...
This data-set is a GPS trajectory of an urban Fisher (*Martes pennati*) with `r length(moveObj@timestamps)` tracks, spanning from `r moveObj@timestamps[1]` to `r moveObj@timestamps[length(moveObj@timestamps)]`, with a mean time interval between tracks of `r round(mean(diff(moveObj@timestamps)),1)` minutes.
...
Including a solar covariate in the clustering is done by means of the parameter *scv* with possible values 'height' or 'azimuth';
...
The output of the *stbc()* constructor is in this case an instance of the *binClstMove* class, and object that simultaneously supports all functionality defined for a *binClstPath* and *Move* objects. Additionally, as we included a covariate, *mybc3* corresponds now to a trivariate binary clustering and therefore its functionality presents some particularities.

Note the output message about the final state of the clustering. Along the optimization process the algorithm can reach a situation where a slight discrepancy arises between the binary and the optimal likelihood partitions, involving just a few data-points at the borders of the clusters. The trade-off between these two solutions can lead the algorithm to get stuck into a cycle. The algorithm itself detects this situation and stops the optimization. We can clearly see this with the likelihood plot;
...
The *sctr()* function of a trivariate clustering shows by default a double scatter-plot for low and high values of the covariate. This can be changed by means of the parameter *showVars=c()*. 
...
If the R-package *rgl* is installed one can use the function *sct3()* to get a dynamic 3D (i.e. can be zoomed and rotated) more useful for a visual understanding of the clusters.
...
This function is indeed defined for and inherited from the *binClst* class, and therefore intended for a general multivariate clustering. If the number of input features is greater than 3, we must use the parameter *showVars=c()* to specify a subset of three variables to be scattered in the plot.
...
The *lblp()* function allows two arguments to compare two output labellings, adding a bottom line indicating the differences;
...
\begin{figure}\centering
  \includegraphics[width=14cm,height=9cm]{pkml0.png}
\caption{Fisher (\textit{Martes pennati}) foraging trajectory. A kml point-wise view of the annotated trajectory showing only the HLH locations. The blob  shows how these locations are spatially clustered most likely indicating the nests.}
\label{pkml0}
\end{figure}

Looking at the spatial distribution of this locations (Figure \ref{pkml0}) and taking into account the HLX labelling (high daytime, low velocity) of the cluster we could tell that these locations are most probably indicating the nests. The package does not deal with labels of the type HLX. However, one can change labels as desired (even to manually force the merging of two clusters). In this case, we would probably feel more comfortable by relabelling the cluster HLH (cluster number 6) as HLL (cluster number 5) to suggest a more clear semantics of *resting* behaviour;
...
Note that the function *rlbl()* does not return a relabelled copy of the input instance, instead it relabels the self instance. Despite of this, the parameters of the clustering remained unchanged. The relabelling is effective only for visualization purposes.
...
To perform the clustering of a stack of trajectories we passed them to the *stbc()* constructor as a list (either of data.frame trajectories, *Move* objects, or a mixture of them);
...
The exception is the *cnfm()* function. This function will work only if expert's labelling is supplied for all trajectories in the stack (in our example, the *moveObj* trajectory does not have expert's labelling);
...
- slot mystckbc@bC is a *binClst* object with the population level clustering, thus it has no path associated, and functions like *view()*, *pkml()* or *bkml()* will not work;

```{r, }
class(mystckbc@bC)
```

- slot mystckbc@bCS is a list of *binClstPath*/*binClstMove* objects, which are the projections of the population level clustering upon each individual;

```{r, }
class(mystckbc@bCS)
```

Each element in the list *mystckbc@bCS* is an instance of the *binClstPath* or *binClstMove* objects in correspondence with the type of data given in the input list;

```{r, }
lapply(mystckbc@bCS,class)
```

It is important to keep this in mind when applying the above functions to either the population (mystckbc@bC, a *binClst* instance) or the individual (mystckbc@bCS[[i]], *binClstPath*/*binClstMove* instances) levels.
...
As usual, it is not necessary to instantiate each individual. For instance, *sctr(slct(mystckbc,1))* yields the same output as *sctr(bcInd1)* or *sctr(mystckbc@bCS[[1]])*;
...
## Comparing individual's behaviour with population's mean behaviour

We can use the functionality of a *binClstPath* object (*e.g.* *sctr()*, *lblp()*, *cnfm()*) to make all kind of comparisons:
